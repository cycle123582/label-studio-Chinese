...
def propagate_annotations(project, queryset, **kwargs):
    ...
    if not annotations:
        # 异常信息建议保持英文，或者是给开发者看的日志；如果是展示给用户看的报错，也可以汉化
    ...
    # 汉化反馈信息
    msg = _('已创建 {} 个标注').format(len(db_annotations))
    return {
        'response_code': 200,
        'detail': msg,
    }
...
def propagate_annotations_form(user, project):
    ...
    # 汉化 label
    label_text = _('输入源标注 ID')
    if first_annotation:
        label_text += f' [{_("首个 ID")}: {str(first_annotation.id)}]'
...
def rename_labels(project, queryset, **kwargs):
    ...
    # 汉化反馈信息
    msg = _('已在 {} 个标注中更新了 {} 个标签').format(annotation_count, label_count)
    return {
        'response_code': 200,
        'detail': msg,
    }
...
def rename_labels_form(user, project):
    ...
    for key, label in labels.items():
        ...
        control_tags.append({'value': key, 'label': key}) # 转为 value/label 格式

    # old_names 也要转为 value/label 格式供 Select 使用
    old_names_options = [{'value': name, 'label': name} for name in list(set(old_names))]
    ...
    return [
        {
            ...
            'fields': [
                ...
                },
                {
                    ...
                    'name': 'old_label_name',
                    'label': _('旧标签名'), # 汉化
                    'options': old_names_options,
                },
                {'type': 'input', 'name': 'new_label_name', 'label': _('新标签名')}, # 汉化
            ],
        }
    ]
    ...
    return [
        {
            'columnCount': 1,
            'fields': [
                {'type': 'input', 'name': 'value_name', 'label': _('名称')}, # 汉化
                {
                    'type': 'select',
                    'name': 'value_type',
                    'label': _('类型'), # 汉化
                    # 必须保留 value 为英文，label 改为中文
                    'options': [
                        {'value': 'String', 'label': _('字符串')},
                        {'value': 'Number', 'label': _('数字')},
                        {'value': 'Expression', 'label': _('表达式')}
                    ],
                },
                {'type': 'input', 'name': 'value', 'label': _('值')}, # 汉化
            ],
        }
    ]
...
actions: list[DataManagerAction] = [
    {
        ...
        'title': _('添加或修改数据字段'), # 汉化
        'order': 1,
        'experimental': True,
        'dialog': {
            'text': _('请确认您想在任务中添加一个新字段。\n'
                      '此操作后，您必须完全刷新数据管理器页面才能看到新列！\n'
                      '您可以使用以下表达式：') + add_data_field_examples, # 汉化
            'type': 'confirm',
    ...
    },
    {
        ...
        'experimental': True,
        'dialog': {
            'text': _('请确认您想在所有标注中重命名标签。\n'
                      '同时，您必须手动在标注配置中更改标签名称。'), # 汉化
            'type': 'confirm',
