    ...
    return (
      ...
      >
        ...
        <div className={cn("sub__result").toClassName()}>
            {/* 使用 t 函数 */}
            {i18next.t("app.all_tasks_completed", "All tasks in the queue have been completed")}
        </div>
        {store.taskHistory.length > 0 && (
          ...
          >
            {/* 使用 t 函数 */}
            {i18next.t("app.go_to_prev_task", "Go to Previous Task")}
          </Button>
        )}
      </div>
    );
    ...
    return (
      <div className={cn("main-view").toClassName()}>
        ...
        {!isFF(FF_DEV_3873) && store.hasInterface("infobar") && (
          // 使用插值 {{id}}
          <div className={cn("main-view").elem("infobar").toClassName()}>
             {i18next.t("app.task_id", { id: store.task.id, defaultValue: `Task #${store.task.id}` })}
          </div>
        )}
      </div>
    );
  ...
  _renderInfobar(as) {
    ...
    return (
      <Space className={cn("main-view").elem("infobar").toClassName()} size="small">
        {/* 使用插值 */}
        <span>{i18next.t("app.task_id", { id: id, defaultValue: `Task #${id}` })}</span>
            ...
            {newUIEnabled ? (
              <InstructionsModal
                ...
                title={
                    ...
                    ? i18next.t("app.review_instructions", "Review Instructions")
                    : i18next.t("app.labeling_instructions", "Labeling Instructions")
                }
              >
                {store.description}
              </InstructionsModal>
