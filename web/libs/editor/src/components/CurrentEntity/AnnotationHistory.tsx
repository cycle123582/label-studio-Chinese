...
const DraftState: FC<{
  ...
  // 汉化 "Me"
  const hiddenUser = infoIsHidden ? { email: t("lsf.history.me", "Me") } : null;
...
const AnnotationHistoryComponent: FC<any> = ({
  ...
  // Default empty state component (汉化空状态)
  const defaultEmptyState = (
    <EmptyState
      icon={<IconHistoryRewind width={24} height={24} />}
      header={t("lsf.history.view_activity", "View annotation activity")}
      description={<>{t("lsf.history.activity_log", "See a log of user actions for this annotation")}</>}
    />
  );
    ...
    <div className={cn("annotation-history").mod({ inline }).toClassName()}>
      ...
      {enabled &&
        ...
        history.map((item: any) => {
          ...
          // 汉化隐藏用户信息时的显示 "Me" / "User"
          const hiddenUser = infoIsHidden
            ? {
                email:
                  currentUser?.id === user.id
                    ? t("lsf.history.me", "Me")
                    : t("lsf.history.user", "User"),
              }
            : null;
...
const HistoryItemComponent: FC<{
  ...
  const reason = useMemo(() => {
    switch (acceptedState) {
      case "accepted":
        return t("lsf.history.accepted", "Accepted");
      case "rejected":
        return t("lsf.history.rejected", "Rejected");
      case "fixed_and_accepted":
        return t("lsf.history.fixed", "Fixed");
      case "updated":
        return t("lsf.history.updated", "Updated");
      case "submitted":
        return t("lsf.history.submitted", "Submitted");
      case "prediction":
        return t("lsf.history.prediction", "From prediction");
      case "imported":
        return t("lsf.history.imported", "Imported");
      case "skipped":
        return t("lsf.history.skipped", "Skipped");
      case "draft_created":
        return t("lsf.history.draft", "Draft");
      case "deleted_review":
        return t("lsf.history.review_deleted", "Review deleted");
      case "propagated_annotation":
        return t("lsf.history.propagated", "Propagated");
      default:
        return null;
    }
  }, [acceptedState, t]); // 添加 t 到依赖
  ...
  return (
    <div className={cn("history-item").elem("comment").mod({ collapsed }).toClassName()} ref={commentRef as any}>
      ...
      {collapsible && (
        ...
        >
          {/* 汉化 Show more / Show less */}
          {collapsed
            ? t("lsf.history.show_more", "Show more")
            : t("lsf.history.show_less", "Show less")
          }
        </div>
      )}
    </div>
