...
// 修复点：移除了 forwardRef 后的泛型 <unknown, ...>，改在参数里定义类型
export const StorageProviderForm = forwardRef(
  (
    {
      onSubmit,
      target,
    ...
    const steps = isEditMode
      ? [
          {
            title: t("storage_settings.steps.configure_connection"),
          ...
          ...(effectiveTarget === "import"
            ? [
                { title: t("storage_settings.steps.import_settings_preview") },
                { title: t("storage_settings.steps.review_confirm") },
              ]
            : []),
        ]
      : [
          { title: t("storage_settings.steps.select_provider"), schema: step1Schema },
          {
            title: t("storage_settings.steps.configure_connection"),
            schema: getProviderSchema(type || "s3", isEditMode, effectiveTarget),
          },
    ...
    useEffect(() => {
      if (onHide) {
        ...
        // This effect depends on onHide prop changes to register/cleanup
        // We rely on the parent component calling onHide when appropriate
      }
    }, [onHide, resetForm]);
