  ...
  const fileChangeHandler: FormEventHandler<HTMLInputElement> = useCallback(
    async (e) => {
      ...
      if (!response.$meta.ok) {
        // 3. 翻译错误信息
        toast?.show({
          message: response?.response?.detail ?? t("user_account.error_updating_avatar", "Error updating avatar"),
  ...
  const userFormSubmitHandler: FormEventHandler = useCallback(
    async (e) => {
      ...
      if (!response?.$meta.ok) {
        // 3. 翻译错误信息
        toast?.show({
          message: response?.response?.detail ?? t("user_account.error_updating_user", "Error updating user"),
          type: ToastType.error
        });
      }
    },
        ...
        <form onSubmit={userFormSubmitHandler} className={styles.sectionContent}>
          <div className={styles.flexRow}>
            <div className={styles.flex1}>
              <Input
                label={t("user_account.first_name", "First Name")}
                value={fname}
            ...
            <div className={styles.flex1}>
              <Input
                label={t("user_account.last_name", "Last Name")}
                value={lname}
          ...
          <div className={styles.flexRow}>
            <div className={styles.flex1}>
              <Input label={t("user_account.email", "E-mail")} type="email" readOnly={true} value={user?.email ?? ""} />
            </div>
            <div className={styles.flex1}>
              <Input
                label={t("user_account.phone", "Phone")}
                type="phone"
...
// === 关键部分：防止白屏报错 ===
// 确保 PersonalInfo 既可以通过名字引用，也可以作为默认导出引用
export const GeneralPage = PersonalInfo;
