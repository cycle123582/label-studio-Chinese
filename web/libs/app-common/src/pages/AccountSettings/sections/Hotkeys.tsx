...
export const HotkeysManager = () => {
  ...
  const getSectionTitle = (sectionId: string): string => {
    ...
    // 如果 section.title 已经是翻译好的（如果后端或defaults没处理），这里可能需要额外翻译
    // 但通常 section.title 也是英文。这里假设 section.title 是 Key 或者需要翻译的英文。
    // 如果 section.title 是如 "Labeling" 这样的英文，你可以用 t(section.title) 尝试翻译，但最好在 defaults 里就定义好 key
  ...
  const handleSaveSection = async (sectionId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        if (toast) {
          toast.show({
            message: t("hotkeys.saved_success", "{{section}} hotkeys saved successfully", { section: sectionName }),
            type: ToastType.info,
          });
        }
      } else {
        if (toast) {
          toast.show({
            message: t("hotkeys.save_failed", "Failed to save: {{error}}", { error: result.error || "Unknown error" }),
      ...
      if (toast) {
        ...
        toast.show({
          message: t("hotkeys.save_error", "Error saving: {{error}}", { error: errorMessage }),
          type: ToastType.error,
        });
      }
    } finally {
  ...
  const handleImportHotkeys = async (importedData: ImportData | Hotkey[]) => {
    try {
      ...
      if (toast) {
        toast.show({ message: t("hotkeys.import_success", "Hotkeys imported successfully"), type: ToastType.info });
      }
    } catch (error: unknown) {
      if (toast) {
        const errorMessage = error instanceof Error ? error.message : "Unknown error";
        toast.show({ message: t("hotkeys.import_error", "Error importing hotkeys: {{error}}", { error: errorMessage }), type: ToastType.error });
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div id="hotkeys-manager">
      ...
      <Dialog open={duplicateConfirmDialog.open} onOpenChange={handleCancelDuplicate}>
        <DialogContent className="bg-neutral-surface">
          <DialogHeader>
            <DialogTitle>{t("hotkeys.duplicate_title", "Warning: Duplicate Hotkey Detected")}</DialogTitle>
            <DialogDescription>
              {/* 这里使用了 Trans 组件或者拆分翻译，简单起见用拆分字符串 */}
              {t("hotkeys.duplicate_desc_start", "The hotkey combination")} "<strong>{duplicateConfirmDialog.newKey}</strong>" {t("hotkeys.duplicate_desc_end", "is already being used by:")}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogDescription className="text-warning-text bg-warning-background p-base rounded-small border border-warning-border-subtle flex items-start gap-tight">
            ...
            <div>
              {t("hotkeys.duplicate_warning", "Having duplicate hotkeys may cause conflicts and unexpected behavior. Are you sure you want to proceed?")}
            </div>
          </DialogDescription>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmDuplicate}>
              {t("hotkeys.allow_duplicate", "Allow Duplicate")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
