...
export const EmailPreferences = () => {
  ...
  const message = useMemo(() => {
    return window.APP_SETTINGS?.whitelabel_is_active
      ? t("email_preferences.subscribe_whitelabel", "Subscribe for news and tips")
      : t("email_preferences.subscribe_heidi", "Subscribe to HumanSignal news and tips from Heidi");
  }, [t]);
          ...
          {Object.entries(emailNotificationSettings).map(([id, { value, label }]: [string, any]) => {
            ...
            return (
              <NotificationCheckbox
                key={id}
                id={id}
                // 尝试翻译后端返回的 Label，如果没有匹配的翻译则显示原文
                label={t(label, label)}
                checked={value}
