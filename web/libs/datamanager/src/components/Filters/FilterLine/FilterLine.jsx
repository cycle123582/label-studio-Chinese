...
const Conjunction = observer(({ index, view }) => {
  ...
  return (
    <FilterDropdown
      items={[
        // 3. 汉化 And / Or
        { value: "and", label: t("filters.and", "And") },
        { value: "or", label: t("filters.or", "Or") },
      ]}
      disabled={index > 1}
...
export const FilterLine = observer(({ filter, availableFilters, index, view, sidebar, dropdownClassName }) => {
  const { t } = useTranslation(); // 4. 初始化翻译
    ...
    return (
      <div className={cn("filterLine").mod({ hasChild: !!childFilter })}>
        ...
        <div className={cn("filterLine").elem("column").mix("conjunction")}>
          {index === 0 ? (
            // 5. 汉化 Where
            <span style={{ fontSize: 12, paddingRight: 5 }}>{t("filters.where", "Where")}</span>
          ) : (
        ...
        <div className={cn("filterLine").elem("column").mix("field")}>
          <FilterDropdown
            placeholder={t("filters.column", "Column")} // 6. 汉化 Column 占位符
        ...
        {childFilter && (
          <>
            ...
            <div className={cn("filterLine").elem("column").mix("conjunction")}>
              {/* 7. 汉化 and (子过滤器连接词) */}
              <span style={{ fontSize: 12, paddingRight: 5 }}>{t("filters.and", "and")}</span>
            </div>
  ...
  return (
    <div className={cn("filterLine").mod({ hasChild: !!childFilter })}>
      ...
      <div className={cn("filterLine").elem("column").mix("field")}>
        <FilterDropdown
          placeholder={t("filters.column", "Column")} // 9. 汉化 Column
          defaultValue={filter.filter.id}
          items={availableFilters}
          width={80}
          dropdownWidth={120}
