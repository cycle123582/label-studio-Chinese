...
const ImportButtonWithChecks = ({ size }) => {
  // 2. 汉化 "Import" 按钮文本
  const importText = i18next.t("data_manager.import", "Import");

  const simpleButton = <ImportButton size={size}>{importText}</ImportButton>;
  ...
  return (
    <Tooltip
      // 3. 汉化 Tooltip 提示
      title={i18next.t("data_manager.upgrade_to_import", "You must upgrade your plan to import data")}
      style={{
        maxWidth: 200,
        textAlign: "center",
      }}
  ...
  columns: ({ size }) => {
    ...
    return (
      <FieldsButton
        ...
        // 4. 汉化 "Columns" 按钮标题
        title={i18next.t("data_manager.columns", "Columns")}
        size={size}
        style={style}
    ...
    return (
      <Interface name="export">
        <ExportButton size={size}>
           {/* 5. 汉化 "Export" 按钮文本 */}
           {i18next.t("data_manager.export", "Export")}
        </ExportButton>
      </Interface>
    );
  },
