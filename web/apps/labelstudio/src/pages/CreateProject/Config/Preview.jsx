// web/src/pages/Projects/Config/Preview.js (已汉化修改)
...
export const Preview = ({ config, data, error, loading, project }) => {
  ...
  useEffect(() => {
    ...
    const lsNode = document.getElementById("label-studio");
    if (lsNode) {
      lsNode.style.opacity = opacity;
    }
  }, [loading, error]);
  ...
  return (
    <div className={configClass.elem("preview")}>
      {/* 已修改: 标题 */}
      <h3>{t("pages.create_project.config.preview_title")}</h3>
      {error && (
