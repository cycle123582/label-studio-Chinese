...
// 将 t 函数作为参数传入，以便在函数内部使用
const initializeDataManager = async (root, props, params, t) => {
  if (!window.LabelStudio) throw Error(t("Label Studio Frontend doesn't exist on the page"));
    ...
    dataManager.on("crash", (details) => {
      ...
      if (isMissingTaskError || isMissingProjectError) {
        const message = isMissingTaskError
          ? t("The task you are trying to access does not exist or is no longer available.")
          : t("The project you are trying to access does not exist or is no longer available.");
